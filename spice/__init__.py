...
class spice(spice_common):
    ...
    def spice_simulator(self):
        ...
        if not hasattr(self, "_spice_simulator"):
            if self.model == "ngspice":
                ...
                self.print_log(
                    type="F", msg=f"Unsupported simulator: {self.model}"
                )
                ...
                try:
                    if targetpath not in iofilepaths:
                        ...
                        if self.preserve_spicefiles:
                            ...
                            if (
                                targetpath == self.spicedbpath
                                and self.interactive_spice
                            ):
                                keepdb = True
                                self.print_log(
                                    type="I",
                                    msg="Preserving %s due to interactive_spice"
                                    % targetpath,
                                )
                                continue
                            ...
                            self.print_log(
                                type="D", msg="Removing %s" % targetpath
                            )
                except:
                    self.print_log(
                        type="W", msg="Could not remove %s" % targetpath
                    )
            if (
                not keepdb
                and not self.preserve_iofiles
                and not self.preserve_spicefiles
            ):
                try:
                    ...
                    if self.model == "eldo":
                        self.print_log(
                            type="I", msg="Waiting for Eldo to exit..."
                        )
                        waittime = 0
                        while (
                            os.system('pgrep "easynch_64.exe" >/dev/null') == 0
                        ):
                            time.sleep(1)
                            waittime += 1
                            if waittime > 60:
                    ...
                    self.print_log(
                        type="D", msg="Removing %s/" % self.spicesimpath
                    )
                except:
                    self.print_log(
                        type="W", msg="Could not remove %s/" % self.spicesimpath
                    )
    ...
    def spicecorner(self):
        ...
        if hasattr(self, "_spicecorner"):
            ...
            self._spicecorner = {"temp": 27, "corner": ""}
        return self._spicecorner
    ...
    def postlayout(self):
        ...
        if not hasattr(self, "_postlayout"):
            if len(self.dspf) > 0:
                self.print_log(
                    type="I",
                    msg="Setting postlayout to True due to given dspf-files",
                )
    ...
    def iofile_eventdict(self):
        ...
        if not hasattr(self, "_iofile_eventdict"):
            ...
            for name, val in self.iofile_bundle.Members.items():
                if (
                    val.dir.lower() == "out" or val.dir.lower() == "output"
                ) and val.iotype == "event":
    ...
    def spice_submission(self):
        ...
        if not hasattr(self, "_spice_submission"):
            try:
                if not self.has_lsf:
                    self.print_log(
                        type="I", msg="LSF not configured, running locally."
                    )
                    ...
                    if self.interactive_spice:
                        if not self.distributed_run:
                            self._spice_submission = (
                                thesdk.GLOBALS["LSFINTERACTIVE"] + " "
                            )
                        else:  # Spectre LSF doesn't support interactive queues
                            self.print_log(
                                type="W",
                                msg="Cannot run in interactive mode if distributed mode is on!",
                            )
                            self._spice_submission = thesdk.GLOBALS[
                                "LSFSUBMISSION"
                            ] + " -o %s/bsublog.txt " % (self.spicesimpath)
                    else:
                        self._spice_submission = thesdk.GLOBALS[
                            "LSFSUBMISSION"
                        ] + " -o %s/bsublog.txt " % (self.spicesimpath)
                ...
                self.print_log(
                    type="W",
                    msg="Error while defining spice submission command. Running locally.",
                )
                self._spice_submission = ""
        ...
        if not hasattr(self, "_spicesrcpath"):
            ...
            try:
                if not (os.path.exists(self._spicesrcpath)):
                    ...
                    self.print_log(
                        type="I", msg="Creating %s." % self._spicesrcpath
                    )
            except:
                self.print_log(
                    type="E", msg="Failed to create %s." % self._spicesrcpath
                )
    ...
    def spicesrc(self):
        ...
        if not hasattr(self, "_spicesrc"):
            self._spicesrc = (
                ...
                + "/"
                + self.name
                + self.spice_simulator.cmdfile_ext
            )

            if not os.path.exists(self._spicesrc):
                self.print_log(
                    type="W",
                    msg="No source circuit found in %s." % self._spicesrc,
                )
    ...
    def spicedbpath(self):
        ...
        if not hasattr(self, "_spicedbpath"):
            self._spicedbpath = (
                ...
                + "/tb_"
                + self.name
                + self.spice_simulator.resultfile_ext
            )
    ...
    def load_output_file(self, value):
        if value:
            self.print_log(
                type="W",
                msg="load_output_file set to True! This will rewrite the entity state on disk!",
            )
    ...
    def connect_spice_inputs(self):
        ...
        for ioname, io in self.IOS.Members.items():
            if ioname in self.iofile_bundle.Members:
                ...
                if (
                    not isinstance(
                        self.IOS.Members[val.name].Data, spice_iofile
                    )
                    and val.dir == "in"
                ):
    ...
    def read_spice_outputs(self):
        ...
        """
        output_iotypes = np.array(
            [
                val.iotype.lower()
                for val in self.iofile_bundle.Members.values()
        ...
        read_output_file = (
            has_event_output | has_sample_output | has_time_output
        )

        if (
            read_output_file
        ):  # First things first, read in the results from the simulation, regardless of iotype
    ...
    def execute_spice_sim(self):
        ...
        self.print_log(
            type="I", msg="Running external command %s" % (self.spicecmd)
        )
        if os.system(self.spicecmd) > 0:
            self.print_log(
                type="E",
                msg="Simulator (%s) returned non-zero exit code."
                % (self.model),
            )
    ...
    def run_plotprogram(self):
        ...
        if not self.distributed_run:
            ...
            self.print_log(
                type="I",
                msg="Waveform viewer %s not launched due to distributed run."
                % self.plotprogram,
            )
    ...
    def extract_powers(self):
        ...
        try:
                ...
                if val.extract and val.sourcetype.lower() == "v":
                    ...
                    if sourcename in self.iofile_eventdict:
                        ...
                        if val.ext_start is not None:
                            arr = arr[
                                np.where(arr[:, 0] >= val.ext_start)[0], :
                            ]
            ...
            if len(self.extracts.Members["powers"].keys()) > 0:
                self.print_log(
                    type="I", msg="Extracted power consumption from transient:"
                )
                # This is newer Python syntax
                maxlen = len(
                    max([*self.extracts.Members["powers"], "total"], key=len)
                )
                for name, val in self.extracts.Members["currents"].items():
                    self.print_log(
                        type="I",
                        msg="%s%s current = %.06f mA"
                        % (name, " " * (maxlen - len(name)), 1e3 * val),
                    )
                if len(self.extracts.Members["currents"].items()) > 0:
                    self.print_log(
                        type="I",
                        msg="Total%s current = %.06f mA"
                        % (
                            " " * (maxlen - 5),
                            1e3
                            * sum(self.extracts.Members["currents"].values()),
                        ),
                    )
                for name, val in self.extracts.Members["powers"].items():
                    self.print_log(
                        type="I",
                        msg="%s%s power   = %.06f mW"
                        % (name, " " * (maxlen - len(name)), 1e3 * val),
                    )
                if len(self.extracts.Members["powers"].items()) > 0:
                    self.print_log(
                        type="I",
                        msg="Total%s power   = %.06f mW"
                        % (
                            " " * (maxlen - 5),
                            1e3 * sum(self.extracts.Members["powers"].values()),
                        ),
                    )
        except:
            ...
            self.print_log(
                type="W",
                msg="Something went wrong while extracting power consumptions.",
            )
    ...
    def use_psf(self, val):
        if self.model != "spectre":
            self.print_log(
                type="W",
                msg=f"use_psf flag is unsupported for simulator {self.model}",
            )
        else:
            self._use_psf = val
    ...
    def run_spice(self):
        ...
        if self.load_state != "":
            ...
            if self.interactive_spice:
                plotthread = threading.Thread(
                    target=self.run_plotprogram, name="plotting"
                )
                plotthread.start()
            ...
            if self.save_state:
                ...
                if self.save_database:
                    try:
                        ...
                        self.print_log(
                            msg="Saving waveform database to %s/%s"
                            % (self.statedir, dbname)
                        )
                    except:
                        self.print_log(
                            type="E",
                            msg="Failed saving waveform database to %s/%s"
                            % (self.statedir, dbname),
                        )
    ...
    def plotlist(self):
        ...
        self.print_log(
            type="O",
            msg="Plotlist has been relocated as an argument to spice_simcmd!",
        )
    ...
    def errpreset(self):
        ...
        self.print_log(
            type="O",
            msg="Errpreset is obsoleted as spectre specific command line argument",
        )
        self.print_log(
            type="O",
            msg="Alternative method for handling simulator specific command line arguments should be developed.",
        )
        ...
        if not hasattr(self, "_analysis_type"):
            ...
            if len(analyses) != 1:
                if len(analyses) < 1:
                    self.print_log(
                        type="W",
                        msg="No analysis type specified! Did you forget to instantiate spice_simcmd object?",
                    )
                    self._analysis_type = ""
                else:
                    self.print_log(
                        type="E",
                        msg="Multiple analyses types specified! Reading output results will most likely fail!",
                    )
