    ...
    def libcmd(self):
        ...
        if not hasattr(self, "_libcmd"):
            ...
            try:
                ...
                self.print_log(
                    type="W", msg="Global TheSDK variable ELDOLIBFILE not set."
                )
                ...
                if val.dir.lower() == "in" or val.dir.lower() == "input":
                    ...
                    if val.iotype.lower() == "event":
                        for i in range(len(val.ionames)):
                            ...
                            # Adding the source
                            self._inputsignals += (
                                "a%s %%vd[%s 0] filesrc%s\n"
                                % (
                                    self.esc_bus(val.ionames[i].lower()),
                                    self.esc_bus(val.ionames[i].upper()),
                                    self.esc_bus(val.ionames[i].lower()),
                                )
                            )
                            self._inputsignals += (
                                '.model filesrc%s filesource (file="%s"\n'
                                % (
                                    self.esc_bus(val.ionames[i].lower()),
                                    os.path.basename(val.file[i]).lower(),
                                )
                            )
                    ...
                    # Presumably these are already converted to bitstrings
                        ...
                        for i in range(len(val.ionames)):
                            ...
                            if (
                                ...
                                self._inputsignals += "a%s [ " % (signame[0])
                                ...
                                for index in range(loopstart, loopstop + 1):
                                    self._inputsignals += "%s_%s_ " % (
                                        signame[0],
                                        index,
                                    )
                                self._inputsignals += "] dac_%s\n" % (
                                    signame[0]
                                )
                                self._inputsignals += (
                                    ".model dac_%s dac_bridge(out_low = %s out_high = %s out_undef = %s input_load = 5.0e-16 t_rise = %s t_fall = %s"
        ...
        if not hasattr(self, "_simcmdstr"):
            ...
            for sim, val in self.simcmds.Members.items():
                if str(sim).lower() == "tran":
                    ...
                    tstep = val.strobeperiod if val.strobeperiod else val.tprint
                    ...
                    if val.noise:
                        self.print_log(
                            type="E",
                            msg=(
                                "Noise transient not available for Ngsim. Running regular transient."
                            ),
                        )
                    ...
                    if val.fscale.lower() == "dec":
                        if val.fpoints != 0:
                            ...
                            self.print_log(
                                type="F",
                                msg="Set fpoints for ngspice AC simulation!",
                            )
            ...
            for name, val in self.simcmds.Members.items():
                ...
                if name.lower() == "tran" or name.lower() == "ac":
                    ...
                    if self.parent.nproc:
                        self._plotcmd += "%s%d\n" % (
                            self.parent.spice_simulator.nprocflag,
                            self.parent.nproc,
                        )
                    self._plotcmd += "run\n"
