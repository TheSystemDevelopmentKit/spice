    ...
    def portsrcstr(self):
        ...
        if not hasattr(self, '_portsrcstr'):
            ...
            for name,port in self.parent.spice_ports.items():
                self.portsrcstr += f"{name} ({port.pos} {port.neg}) port num={port.num} r={port.res} x={port.reactance} type={port.type} freq={port.freq} mag={port.mag} dc={port.dc}\n"
        ...
        if not hasattr(self,'_simcmdstr'):
            ...
            for sim, val in self.simcmds.Members.items():
                ...
                if str(sim).lower() == 'tran':
                    ...
                    self._simcmdstr += 'Initial_analysis pss fund={val.fc} outputtype=freq maxacfreq={val.fmax} harms={val.harmonics}\n'
                    self._simcmdstr += 'PAC_analysis pac values=[{val.fsig}] maxsideband={val.harmonics}'
                    ...
                    self._simcmdstr += 'PSS_analysis pss fund={val.fsig} outputtype=freq maxacfreq={val.fmax} harms={val.harmonics}'
                else:
