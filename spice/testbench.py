...
class testbench(testbench_common):
    ...
    def testbench_simulator(self):
        ...
        if not hasattr(self, "_testbench_simulator"):
            if self.model == "ngspice":
                self._testbench_simulator = ngspice_testbench(
                    parent=self.parent
                )
            if self.model == "eldo":
                self._testbench_simulator = eldo_testbench(parent=self.parent)
            if self.model == "spectre":
                self._testbench_simulator = spectre_testbench(
                    parent=self.parent
                )
    ...
    def parameters(self):
        ...
        if not hasattr(self, "_parameters"):
            ...
            for parname, parval in self.parent.spiceparameters.items():
                self._parameters += (
                    self.parent.spice_simulator.parameter
                    + " "
                    + str(parname)
                    + "="
                    + str(parval)
    ...
    def includecmd(self):
        ...
        if not hasattr(self, "_includecmd"):
            ...
            self._includecmd += '%s "%s"\n' % (
                self.parent.spice_simulator.include,
                self._subcktfile,
            )
    ...
    def copy_dspf(self):
        try:
            ...
            self.print_log(
                type="F",
                msg="Could not copy DSPF for cell %s to %s" % (cell, dest),
            )
    ...
    def dspfincludecmd(self):
        ...
        if not hasattr(self, "_dspfincludecmd"):
            if len(self.parent.dspf) > 0:
                self.copy_dspf()
                self.print_log(
                    type="I", msg="Including exctracted parasitics from DSPF."
                )
                    ...
                    try:
                        ...
                        with open(dspfpath) as dspffile:
                            ...
                            for line in lines:
                                ...
                                if origcellmatch.search(line) != None:
                                    ...
                                    if (
                                        ...
                                        self.print_log(
                                            type="I",
                                            msg="Found DSPF cell name matching to original top-level cell name.",
                                        )
                                        found = True
                                        self._origcellname = cellname
                                    elif (
                                        cellname.lower()
                                        == self.dut.custom_subckt_name.lower()
                                    ):
                                        self.print_log(
                                            type="I",
                                            msg="Found DSPF cellname matching to custom_subckt_name: %s."
                                            % cellname,
                                        )
                                        found = True
                                        self._origcellname = cellname
                            ...
                            if found:
                                ...
                                self.print_log(
                                    type="I",
                                    msg='Renaming DSPF top cell name accordingly from "%s" to "%s".'
                                    % (cellname, self.parent.name),
                                )
                                with fileinput.FileInput(
                                    dspfpath, inplace=True, backup=".bak"
                                ) as f:
                                    for line in f:
                                        print(
                                            line.replace(
                                ...
                                self.print_log(
                                    type="F",
                                    msg="No DESIGN string in DSPF matching %s or %s. Aborting"
                                    % (
                                        self.parent.name,
                                        self.dut.custom_subckt_name,
                                    ),
                                )

                            self.print_log(
                                type="I",
                                msg="Including DSPF-file: %s" % dspfpath,
                            )
                            self._dspfincludecmd += '%s "%s"\n' % (
                                self.parent.spice_simulator.dspfinclude,
                                dspfpath,
                            )
                    except:
                        self.print_log(
                            type="F",
                            msg="No DESIGN string in DSPF matching %s or %s. Aborting"
                            % (self.parent.name, self.dut.custom_subckt_name),
                        )
                        self.print_log(type="F", msg=traceback.format_exc())
            else:
                self._dspfincludecmd = ""
                if len(self.parent.postlayout_subckts) > 0:
                    self.print_log(
                        type="I",
                        msg="Including exctracted parasitics from subcircuit DSPF.",
                    )
                    self._dspfincludecmd = (
                        "%s Extracted subcircuit parasitics\n"
                        % self.parent.spice_simulator.commentchar
                    )
                    for dspf in self.parent.postlayout_subckts:
                        dspfpath = "%s/%s.pex.dspf" % (
                            self.parent.spicesrcpath,
                            dspf,
                        )
                        if os.path.exists(dspfpath):
                            self.print_log(
                                type="I",
                                msg="Including subcircuit DSPF-file: %s"
                                % dspfpath,
                            )
                            self._dspfincludecmd += '%s "%s"\n' % (
                                self.parent.spice_simulator.dspfinclude,
                            ...
                            self.print_log(
                                type="W",
                                msg="No such file or directory %s." % dspfpath,
                            )
    ...
    def export(self, **kwargs):
        ...
        if (
            ...
            and len(self.parent.dspf) == 0
            and len(self.parent.postlayout_subckts) == 0
        ):
            self.print_log(
                type="W",
                msg="No top-cell dspf or subcircuit dspf for postlayout simulation. Are you using some other netlist format?",
            )
            self.print_log(
                type="W",
                msg=f"Exporting subcircuit, but simulation is not postlayout unless {self.parent.spicesrc} is postlayout netlist!",
            )
        self.dut.export_subckts(file=self._subcktfile, force=force)

        if not os.path.isfile(self.file):
            self.print_log(
                type="D", msg="Exporting spice testbench to %s" % (self.file)
            )
            ...
            self.print_log(
                type="I",
                msg="Forcing overwrite of spice testbench to %s." % (self.file),
            )
            with open(self.file, "w") as module_file:
                module_file.write(self.contents)
