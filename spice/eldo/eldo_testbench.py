...
class eldo_testbench(testbench_common):
    ...
    def portsrcstr(self):
        ...
        if not hasattr(self, "_portsrcstr"):
            self.portsrcstr = ""
            self.print_log(
                type="W", msg="Port support not yet implemented for Eldo!"
            )
                ...
                if val.dir.lower() == "in" or val.dir.lower() == "input":
                    ...
                    # Presumably these are already converted to bitstrings
                        ...
                        for i in range(len(val.ionames)):
                            ...
                            # Adding the source
                            self._inputsignals += (
                                ".sigbus %s vhi=%s vlo=%s tfall=%s trise=%s thold=%s tdelay=%s base=%s PATTERN %s\n"
                                % (
                                    busname,
                                    str(val.vhi),
                                    str(val.vlo),
        ...
        if not hasattr(self, "_simcmdstr"):
            ...
            for sim, val in self.simcmds.Members.items():
                if str(sim).lower() == "tran":
                    ...
                    if val.noise:
                        self._simcmdstr += (
                            ".noisetran fmin=%s fmax=%s nbrun=1 NONOM %s\n"
                            % (
                                str(val.fmin),
                                str(val.fmax),
                                (
                    ...
                    print_log(
                        type="F",
                        msg="AC simulation for eldo not yet implemented",
                    )
                    self._simcmdstr += "\n\n"
                else:
                    self.print_log(
