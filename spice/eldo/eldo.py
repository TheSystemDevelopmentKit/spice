    ...
    **kwargs :
    ...
    def spicecmd(self):
        ...
        if not hasattr(self, "_spicecmd"):
            ...
            if self.parent.postlayout:
                self.print_log(
                    type="W",
                    msg="Post-layout optimization not suported for Eldo",
                )
        ...
        try:
            ...
            if ret != 0:
                self.print_log(
                    ...
                    % (self.parent.plotprogram, ret),
                )
        except:
            self.print_log(
                type="W",
                msg="Something went wrong while launcing %s."
                % self.parent.plotprogram,
            )
    ...
    def read_output_file(self, file, dtype):
        ...
        if len(header) != len(arr[0, :]):
            self.print_log(
                type="E",
                msg="Signal name and array column mismatch while reading event outputs.",
            )
        for col_idx, sname in enumerate(header[1:]):
            ...
            if label:
                ...
                self.iofile_eventdict[label.upper()] = np.hstack(
                    (
                        arr[:, 0].reshape(-1, 1),
                        arr[:, col_idx + 1].reshape(-1, 1),
                    )
                ).reshape(-1, 2)
            else:
                self.print_log(
                    type="W", msg="Label format mismatch with '%s'." % (label)
                )
