...
class spice_module(thesdk):
    ...
    def __init__(self, **kwargs):
        ...
        self.parent = kwargs.get(
            "parent"
        )  # There should be no need for parent in this module
    ...
    def subckt(self):
        ...
        if not hasattr(self, "_subckt"):
            ...
            if os.path.isfile(self.file):
                try:
                    self.print_log(
                        type="D", msg="Parsing source netlist %s" % self.file
                    )
                    ...
                    self.print_log(
                        type="E",
                        msg="Something went wrong while parsing %s."
                        % self.file,
                    )
        ...
        try:
            if not hasattr(self, "_instance"):
                ...
                startmatch = re.compile(
                    r"%s %s "
                    % (self.parent.spice_simulator.subckt, self.parent.name),
                    re.IGNORECASE,
                )
                ...
                if len(subckt) <= 3:
                    ...
                    for line in subckt:
                        ...
                        if startfound and not endfound:
                            ...
                            if (
                                words[0].lower()
                                == self.parent.spice_simulator.subckt
                            ):
                                if self.parent.model == "eldo":
            ...
            self.print_log(
                type="E",
                msg="Something went wrong while generating subcircuit instance.",
            )
            self.print_log(type="E", msg=traceback.format_exc())
    ...
    def export_subckts(self, **kwargs):
        ...
        if not os.path.isfile(file):
            self.print_log(
                type="D", msg="Exporting spice subcircuit to %s" % (file)
            )
            ...
            self.print_log(
                type="I",
                msg="Forcing overwrite of spice subcircuit to %s." % (file),
            )
            with open(file, "w") as module_file:
                module_file.write(self.subckt)
