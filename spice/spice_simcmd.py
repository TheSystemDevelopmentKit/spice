...
class spice_simcmd(thesdk):
    ...
    **kwargs :  
            ...
            where: str
                DC only. Where to save DC operating point results. 'file',
                'screen' and 'logfile' are valid options. In case 'file' is
                specified, the filename is set by 'filename' property.
            filename: str
                DC only. The filename of the DC operating point result file, if
                where == 'file'.
            as_prop: bool
                DC & Spectre models only. If set as True, extracts the values
                specified in the plotlist, and sets them in self.parent.oppts
