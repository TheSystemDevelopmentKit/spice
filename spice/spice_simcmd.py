...
class spice_simcmd(thesdk):
    ...
    def __init__(self, parent, **kwargs):
        try:
            ...
            self.sweep = (
                kwargs.get("sweep", [])
                if type(kwargs.get("sweep", [])) == list
                else [kwargs.get("sweep")]
            )
            self.subcktname = (
                kwargs.get("subcktname", [])
                if type(kwargs.get("subcktname", [])) == list
                else [kwargs.get("subcktname")]
            )
            self.devname = (
                kwargs.get("devname", [])
                if type(kwargs.get("devname", [])) == list
                else [kwargs.get("devname")]
            )
            self.swpstart = (
                kwargs.get("swpstart", [])
                if type(kwargs.get("swpstart", [])) == list
                else [kwargs.get("swpstart")]
            )
            self.swpstop = (
                kwargs.get("swpstop", [])
                if type(kwargs.get("swpstop", [])) == list
                else [kwargs.get("swpstop")]
            )
            self.swpstep = (
                kwargs.get("swpstep", [])
                if type(kwargs.get("swpstep", [])) == list
                else [kwargs.get("swpstep")]
            )
            self.swpvalues = (
            ...
            self.nodes = (
                kwargs.get("nodes", [])
                if type(kwargs.get("nodes", [])) == list
                else [kwargs.get("nodes")]
            )
        except:
        ...
        if (
            self.sim in ["dc", "sp", "stb", "pz", "pss", "pac", "noise"]
        ...
        if len(self.subcktname) != 0 and len(self.devname) != 0:
            self.print_log(
                type="F",
                msg="Cannot specify subckt sweep and device sweep in the same simcmd instance!",
            )
        if self.strobeperiod and self.strobedelay:
            if self.strobedelay > self.strobeperiod:
                self.print_log(
                    type="F",
                    msg="Strobedelay cannot be larger than strobeperiod!",
                )
